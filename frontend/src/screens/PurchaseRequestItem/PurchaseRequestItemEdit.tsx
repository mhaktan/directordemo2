...
interface PurchaseRequestItemEditProps {
  record: PurchaseRequestItemRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const PurchaseRequestItemEdit: React.FC<PurchaseRequestItemEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<PurchaseRequestItemRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<PurchaseRequestItemRecord>) =>
      dataProvider.update('PurchaseRequestItem', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'PurchaseRequestItem', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit PurchaseRequestItem</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <TkInput mode="text" label="Product Name *" value={String(form.productName ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('productName', v))(e.detail)} />
                </div>
                <div>
                  <TkInput mode="number" label="Quantity *" value={String(form.quantity ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('quantity', Number(v)))(e.detail)} />
                </div>
                <div>
                ...
                <div>
                  <TkInput mode="number" label="Line Total" value={String(form.lineTotal ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('lineTotal', Number(v)))(e.detail)} />
                </div>
                <div>
