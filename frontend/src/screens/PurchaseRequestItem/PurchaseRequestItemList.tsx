...
import { PurchaseRequestItemCreate } from './PurchaseRequestItemCreate';
import { PurchaseRequestItemEdit } from './PurchaseRequestItemEdit';
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'productName', header: "Product Name", sortable: true, searchable: true, filterType: 'text' },
  { field: 'quantity', header: "Quantity", sortable: true },
  { field: 'unitPrice', header: "Unit Price", sortable: true },
  { field: 'lineTotal', header: "Line Total", sortable: true },
...
export const PurchaseRequestItemList: React.FC = () => {
  const list = useListQuery<PurchaseRequestItemRecord>({ resource: 'PurchaseRequestItem' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<PurchaseRequestItemRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<PurchaseRequestItemRecord[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.resolve(dataProvider.getOne('PurchaseRequestItem', focusId))
      .then((rec) => { if (rec) setEditRecord(rec as PurchaseRequestItemRecord); })
      .catch(() => { /* kayit bulunamadi — liste normal acilir */ })
      .finally(clear);
  }, [focusId]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('PurchaseRequestItem', (ids) => { ids.forEach(id => triggerFlows('delete', 'PurchaseRequestItem', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as PurchaseRequestItemRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create PurchaseRequestItem" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <PurchaseRequestItemCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <PurchaseRequestItemEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
