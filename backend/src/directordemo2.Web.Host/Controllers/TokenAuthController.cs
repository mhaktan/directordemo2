...
using directordemo2.Authentication;
using directordemo2.Entities;
using directordemo2.EntityFrameworkCore;

namespace directordemo2.Web.Host.Controllers
{
        ...
        public ActionResult Authenticate([FromBody] AuthenticateModel model)
        {
            try
            {
                ...
                if (user.PasswordHash != null && user.PasswordHash.StartsWith("PBKDF2$"))
                    pwOk = directordemo2.Users.PasswordHelper.VerifyPbkdf2(model.Password, user.PasswordHash);
                else
