...
namespace directordemo2.EntityFrameworkCore.Seed
{
    /// <summary>
    ...
    public class DefaultUserSeeder
    {
        ...
        public void Seed()
        {
            ...
            try
            {
                ...
                Console.WriteLine("[Seed] Step 5: Custom role 'Requester'");
                var customRole_0 = _roleRepo.GetAll().FirstOrDefault(r => r.Name == "Requester");
                if (customRole_0 == null)
                {
                    customRole_0 = new AppRole { Name = "Requester", DisplayName = "Talep Eden", Description = "Satın alma talebi oluşturur, kalem ekler ve süreç sonucunu takip eder.", IsSystem = false, IsActive = true };
                    customRole_0.Id = _roleRepo.InsertAndGetId(customRole_0);
                    Console.WriteLine($"[Seed] Created custom role 'Requester' (id={customRole_0.Id})");
                }
                var customPerms_0 = new string[] { "PurchaseRequest.Create", "PurchaseRequest.Read", "PurchaseRequest.Update", "PurchaseRequestItem.Create", "PurchaseRequestItem.Read", "PurchaseRequestItem.Update", "PurchaseRequestItem.Delete", "Quotation.Read" };
                ...
                {
                    if (!allPermSet_0.Contains(pn)) { Console.WriteLine($"[Seed] Skipping unknown permission '{pn}' for role 'Requester'"); continue; }
                    _rolePermRepo.Insert(new RolePermission { RoleId = customRole_0.Id, PermissionName = pn });
                }

                Console.WriteLine("[Seed] Step 6: Custom role 'DepartmentManager'");
                var customRole_1 = _roleRepo.GetAll().FirstOrDefault(r => r.Name == "DepartmentManager");
                if (customRole_1 == null)
                {
                    customRole_1 = new AppRole { Name = "DepartmentManager", DisplayName = "Birim Müdürü", Description = "Biriminden gelen talepleri inceler, onaylar veya reddeder.", IsSystem = false, IsActive = true };
                    customRole_1.Id = _roleRepo.InsertAndGetId(customRole_1);
                    Console.WriteLine($"[Seed] Created custom role 'DepartmentManager' (id={customRole_1.Id})");
                }
                var customPerms_1 = new string[] { "PurchaseRequest.Read", "PurchaseRequest.ChangeStatus", "PurchaseRequestItem.Read", "Quotation.Read", "Employee.Read", "Department.Read" };
                ...
                {
                    if (!allPermSet_1.Contains(pn)) { Console.WriteLine($"[Seed] Skipping unknown permission '{pn}' for role 'DepartmentManager'"); continue; }
                    _rolePermRepo.Insert(new RolePermission { RoleId = customRole_1.Id, PermissionName = pn });
                }

                Console.WriteLine("[Seed] Step 7: Custom role 'FinanceApprover'");
                var customRole_2 = _roleRepo.GetAll().FirstOrDefault(r => r.Name == "FinanceApprover");
                if (customRole_2 == null)
                {
                    customRole_2 = new AppRole { Name = "FinanceApprover", DisplayName = "Finans Onaycısı", Description = "Müdür onaylı talepleri finans açısından değerlendirir, onaylar veya reddeder.", IsSystem = false, IsActive = true };
                    customRole_2.Id = _roleRepo.InsertAndGetId(customRole_2);
                    Console.WriteLine($"[Seed] Created custom role 'FinanceApprover' (id={customRole_2.Id})");
                }
                var customPerms_2 = new string[] { "PurchaseRequest.Read", "PurchaseRequest.ChangeStatus", "PurchaseRequestItem.Read", "Quotation.Read", "Department.Read", "ExpenseCategory.Read" };
                ...
                {
                    if (!allPermSet_2.Contains(pn)) { Console.WriteLine($"[Seed] Skipping unknown permission '{pn}' for role 'FinanceApprover'"); continue; }
                    _rolePermRepo.Insert(new RolePermission { RoleId = customRole_2.Id, PermissionName = pn });
                }

                Console.WriteLine("[Seed] Step 8: Custom role 'PurchasingOfficer'");
                var customRole_3 = _roleRepo.GetAll().FirstOrDefault(r => r.Name == "PurchasingOfficer");
                if (customRole_3 == null)
                {
                    customRole_3 = new AppRole { Name = "PurchasingOfficer", DisplayName = "Satın Alma Sorumlusu", Description = "Onaylanan taleplere teklif ekler, tedarikçi seçer, siparişi oluşturur ve onaylayarak teslimat sürecini yönetir.", IsSystem = false, IsActive = true };
                    customRole_3.Id = _roleRepo.InsertAndGetId(customRole_3);
                    Console.WriteLine($"[Seed] Created custom role 'PurchasingOfficer' (id={customRole_3.Id})");
                }
                var customPerms_3 = new string[] { "PurchaseRequest.Read", "PurchaseRequest.Create", "PurchaseRequest.Update", "PurchaseRequest.Delete", "PurchaseRequest.ChangeStatus", "PurchaseRequestItem.Read", "PurchaseRequestItem.Create", "PurchaseRequestItem.Update", "PurchaseRequestItem.Delete", "PurchaseRequestItem.ChangeStatus", "Quotation.Read", "Quotation.Create", "Quotation.Update", "Quotation.Delete", "Quotation.ChangeStatus", "PurchaseOrder.Read", "PurchaseOrder.Create", "PurchaseOrder.Update", "PurchaseOrder.Delete", "PurchaseOrder.ChangeStatus", "Supplier.Read", "ExpenseCategory.Read", "Department.Read", "Employee.Read" };
                ...
                {
                    if (!allPermSet_3.Contains(pn)) { Console.WriteLine($"[Seed] Skipping unknown permission '{pn}' for role 'PurchasingOfficer'"); continue; }
